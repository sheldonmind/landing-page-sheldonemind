...
/** The width the mock is composed for: al-container's widest inner width (its 1440px max-width
 *  minus 40px inline padding either side). The panels are fixed px while the render card is
 *  sized as a % of this, so the composition only holds at its design width — narrower and the
 *  card gets too short for the 452px chat panel, which then spills onto the shot library bar.
 *  So rather than let it reflow at all, it is pinned here and scaled to fit. */
const DESIGN_W = 1360;
  ...
  return (
    <section id="drama-studio" className="al-section relative w-full overflow-hidden">
      ...
      <div className="al-container relative z-10 mt-16 max-md:mt-12">
        <div ref={wrapRef}>
          <div ref={mockRef} className="relative pb-14">
            {/* Render card. */}
            <div className="relative ml-auto w-[84%] overflow-hidden rounded-2xl border border-white/10">
              <video
            ...
            <div className="absolute bottom-0 right-3 block w-[calc(84%-1.5rem)] rounded-xl border border-white/15 bg-transparent px-4 py-3 backdrop-blur-md">
            ...
            <div className="absolute left-0 top-10 flex h-[452px] w-[336px] flex-col overflow-hidden rounded-2xl border border-white/12 bg-black/60 shadow-[0_24px_70px_rgba(0,0,0,0.6)] backdrop-blur-xl">
              {/* Header. */}
